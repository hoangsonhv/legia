...
class WarehouseSupplyController extends Controller
{
    ...
    function __construct(
        ...
        $this->menu                     = [
            'root' => 'Quản lý Kho Vật Dụng',
            'data' => [
                'parent' => [
                    'href'   => route('admin.warehouse-supply.index'),
                    'label'  => 'Quản lý Kho Vật dụng'
                ]
            ]
        ];
    }
    ...
    public function index(Request $request, $model=null)
    {
        ...
        $types         = DataHelper::getModelWarehouses('supply');
        ...
        $request->flash();
        return view('admins.warehouse_supplies.index',compact('types', 'breadcrumb', 'titleForLayout', 'warehouseRemains', 'model'));
    }
    ...
    public function create($model)
    {
        ...
        return view('admins.warehouse_supplies.create',compact('breadcrumb', 'titleForLayout', 'permissions', 'model'));
    }
    ...
    public function store(WarehouseRemainRequest $request, $model)
    {
        ...
        if ($this->warehouseService->storeOrUpdate($model, $input)) {
            return redirect()->route('admin.warehouse-supply.index', ['model' => $model])->with('success','Tạo Vật Liệu thành công!');
        }
        return redirect()->route('admin.warehouse-supply.index', ['model' => $model])->with('error','Tạo Vật Liệu thất bại!');
    }
    ...
    public function edit(Request $request, $model, $id)
    {
        ...
        if ($warehouseRemain = $this->warehouseService->edit($id,$model)) {
            $permissions = config('permission.permissions');
            return view('admins.warehouse_supplies.edit',compact('breadcrumb', 'titleForLayout', 'warehouseRemain', 'permissions', 'model'));
        }
        return redirect()->route('admin.warehouse-supply.index', ['model' => $model])->with('error', 'Vật Liệu không tồn tại!');
    }
    ...
    public function update(WarehouseRemainRequest $request, $model, $id)
    {
        ...
        if ($this->warehouseService->storeOrUpdate($model,$input)) {
            ...
            return redirect()->route('admin.warehouse-supply.edit', ['model' => $model, 'id' => $id])->with('success','Cập nhật Vật Liệu thành công!');
        }
    ...
    public function destroy($model, $id)
    {
        ...
        if ($this->warehouseService->delete($id,$model)) {
            return redirect()->route('admin.warehouse-supply.index', ['model' => $model])->with('success','Xóa Vật Liệu thành công!');
        }
        return redirect()->back()->with('error', 'Vật Liệu không tồn tại!');
    }
    ...
    public function import(Request $request, $model) {
        ...
        try{
            set_time_limit(0);
            $file = $request->file('import-supply');
    ...
    private function checkExistModel($data) {
        $typeModels = DataHelper::getModelWarehouses('supply');
