...
class ManufactureRepository extends AdminRepository
{
    ...
    public function updateIsCompleted($id)
    {
        $query = ManufactureDetail::where('manufacture_id', $id)
            ->whereRaw('COALESCE(reality_quantity, 0) < manufacture_quantity')
            ->get();
        ...
        if(!$query->count()) {
            ...
            foreach ($details as $index => $detail) {
                ...
                if($base_warehouse != null) {
                    ...
                    $merchandise->size = $detail->size;
                    $merchandise->dia_w_w1 = $detail->dia_w_w1;
                    $merchandise->l_l1 = $detail->l_l1;
                    ...
                    if ($base_warehouse != null) {
                        ...
                        $new_merchandise->size = $detail->size;
                        $new_merchandise->dia_w_w1 = $detail->dia_w_w1;
                        $new_merchandise->l_l1 = $detail->l_l1;
